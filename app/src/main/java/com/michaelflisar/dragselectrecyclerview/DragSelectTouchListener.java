...
/**
 * Link to original: https://github.com/MFlisar/DragSelectRecyclerView.git
 * Changes:
 * 1.Replaced package android to androidx for RecyclerView and ViewCompat
 * 2.Update deprecated class
 */
public class DragSelectTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    @Override
    public boolean onInterceptTouchEvent(@NotNull RecyclerView rv, @NotNull MotionEvent e) //add @NotNull
    {
        if (!mIsActive || rv.getAdapter().getItemCount() == 0)
            return false;

        int action = e.getActionMasked(); //// replaced MotionEventCompat.getActionMasked(e) with e.getActionMasked()
        switch (action) {
    ...
    @Override
    public void onTouchEvent(@NotNull RecyclerView rv, @NotNull MotionEvent e) //add @NotNull
    {
        if (!mIsActive)
            return;
